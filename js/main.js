...
// ============================================
// HERO SEARCH — Smart Dropdown (body-appended)
// ============================================
function initToolSearch() {
  const searchInput = document.getElementById('heroSearch');
  if (!searchInput) return;

  // Append dropdown to BODY to escape hero overflow:hidden
  let dropdown = document.getElementById('heroSearchDropdown');
  if (!dropdown) {
    dropdown = document.createElement('div');
  ...
  function positionDropdown() {
    const rect = searchInput.closest('.hero-search-wrapper').getBoundingClientRect();
  ...
  function buildItems(tools) {
    return tools.map(t =>
      `<a href="${t.url}" class="search-result-item" role="option" tabindex="0">
        <span class="sri-icon" aria-hidden="true">${t.icon}</span>
        <span class="sri-info">
          <span class="sri-name">${t.name}</span>
          <span class="sri-cat">${t.category}</span>
        </span>
  ...
  function applyDropdownTheme() {
    // Light theme only (dark mode removed from this build)
    ...
    dropdown.querySelectorAll('.search-result-item').forEach(item => {
      ...
      item.style.borderBottomColor = '#E2E8F0';
      const name = item.querySelector('.sri-name');
      const cat  = item.querySelector('.sri-cat');
      const icon = item.querySelector('.sri-icon');
      const arrow = item.querySelector('.sri-arrow');
      if (name)  name.style.color  = '#1E293B';
      if (cat)   cat.style.color   = '#94A3B8';
      if (icon)  icon.style.background = '#F8FAFC';
      if (arrow) arrow.style.color = '#94A3B8';
    });

    const header = dropdown.querySelector('.search-popular-header');
    if (header) {
    ...
    const noResult = dropdown.querySelector('.search-no-result');
    if (noResult) noResult.style.color = '#64748B';
  }
  ...
  // Hover highlight for result items
  ...
  searchInput.addEventListener('input', () => {
    ...
    if (!q) {
      showDropdown(
        '<div class="search-popular-header">Popular Calculators</div>' +
        buildItems(TOOLS.slice(0, 6))
      );
      return;
    }
    const matches = TOOLS.filter(t =>
    ...
    if (!matches.length) {
      showDropdown('<div class="search-no-result">No results for "<strong>' + q + '</strong>"</div>');
    } else {
      showDropdown(buildItems(matches.slice(0, 7)));
    }
  });
  ...
  // Search button click
  const searchBtn = searchInput.closest('.hero-search')?.querySelector('button');
  if (searchBtn) {
  ...
  searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const first = dropdown.querySelector('a.search-result-item');
  ...
  dropdown.addEventListener('keydown', (e) => {
    const items = Array.from(dropdown.querySelectorAll('a.search-result-item'));
    const idx = items.indexOf(document.activeElement);
    if (e.key === 'ArrowDown') { e.preventDefault(); (items[idx + 1] || items[0]).focus(); }
    if (e.key === 'ArrowUp')   { e.preventDefault(); (items[idx - 1] || searchInput).focus(); }
    if (e.key === 'Escape')    { hideDropdown(); searchInput.focus(); }
  });

  // Mobile: use touchend + mousedown to navigate (not click — avoids focus-blur race)
  dropdown.addEventListener('mousedown', (e) => {
    const link = e.target.closest('a.search-result-item');
    if (link) {
      e.preventDefault(); // prevent blur from firing before navigation
      window.location.href = link.getAttribute('href');
    }
  });
  // Track if user is scrolling (prevent click on scroll)
  let touchStartY = 0;
  ...
  dropdown.addEventListener('touchmove', (e) => {
    ...
    if (deltaY > 8) isTouchScrolling = true; // 8px threshold
  }, { passive: true });

  dropdown.addEventListener('touchend', (e) => {
    if (isTouchScrolling) return; // ignore if was scrolling
    const link = e.target.closest('a.search-result-item');
    if (link) {
      e.preventDefault();
      window.location.href = link.getAttribute('href');
  ...
  document.addEventListener('mousedown', (e) => {
    const wrapper = searchInput.closest('.hero-search-wrapper');
  ...
  // Also filter homepage tool cards
